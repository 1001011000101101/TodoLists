...
ejs.createSelectOptions = function (set, wheres, orders, take, skip, includes, whereMethod, orderMethod, mode) {
    var options = {
        entitySetName: ejs.toServerName(set.settings.name),
        entityName: ejs.toServerName(set.settings.className),
        selectMode: mode || ejs.toServerName(set.settings.mode),
...
ejs.toJsObject = function (value) {
    ...
    for (var i in value) {
        ...
        if (v == null || v == undefined) {
            ...
            v = parseDate(v).toSds() + " " + parseDate(v).toSts();
        }
...
ejs.getFileDownloadPath = function (n, id) {
    var r = ejs.arp() + "Data/DownloadFile?ID=" + n + "&FileID=" + id;
    return r;
};
